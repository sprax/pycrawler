...
# global variable for start time; used with _log(..., relative=True)
start = 0
...
def log(msg):
    """Log an error message, based on syslog_open"""
    if syslog_type:
        print >> syslog_type, msg
        syslog_type.flush()
    else:
        syslog.syslog(syslog.LOG_ERR, msg)

def _log(msg, fh=sys.stderr, timestamp=True, relative=True):
    """
...
def log(rec, level=1, verbosity=0):
    """
    If level is *below* the verbosity, then the rec gets passed to
    _construct_message and logged with _log.  Note that this log
    method has rec first and it is not optional; contrast with
    ChangeLogger.log()
    """
    if level <= verbosity:
        _log(_construct_message(rec))
...
if __name__ == "__main__":
    ...
    def mlog( msg ):
        log( ("__main__", msg), level=0, verbosity=3 )
